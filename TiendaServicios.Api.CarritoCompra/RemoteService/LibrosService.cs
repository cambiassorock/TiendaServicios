...
namespace TiendaServicios.Api.CarritoCompra.RemoteService
{
    public class LibrosService : ILibrosService
    {
        ...
        public async Task<(bool resultado, LibroMaterialRemote libroRemote, string ErrorMessage)> GetLibro(Guid LibroId)
        {
            try 
            {
                var cliente = _httpClient.CreateClient("Libros");
                var response = await cliente.GetAsync($"api/libro/{LibroId.ToString()}");
                ...
                if(response.IsSuccessStatusCode)
                {
                    ...
                    var resultado = JsonSerializer.Deserialize<LibroMaterialRemote>(contenido, options);
